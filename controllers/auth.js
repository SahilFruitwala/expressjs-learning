...
exports.postSignup = (req, res, next) => {
  ...
  bcrypt
    ...
    .catch((err) => {
      const error = new Error("Error while signing up!\n", err);
      error.httpStatusCode = 500;
      return next(error);
    });
};
...
exports.postReset = (req, res, next) => {
  ...
  crypto.randomBytes(32, (err, buffer) => {
    ...
    User.findOne({ email: email })
      ...
      .catch((err) => {
       const error = new Error("Error in password reset!\n", err);
       error.httpStatusCode = 500;
       return next(error);
      });
  });
...
exports.getNewPassword = (req, res, next) => {
  ...
  User.findOne({
    ...
    .catch((err) => {
      const error = new Error("Error in new password!\n", err);
      error.httpStatusCode = 500;
      return next(error);
    });
};
...
exports.postNewPassword = (req, res, next) => {
  ...
  User.findOne({
    ...
    .catch((err) => {
      const error = new Error("Error in post new password!\n", err);
      error.httpStatusCode = 500;
      return next(error);
    });
};
